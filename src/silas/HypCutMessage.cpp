...
{
    if ((run == 0) && !(m_reg_msg_size.empty() && m_reg_msg_tag.empty()))
    {
...
void HypCutMessage::remove_message(const pearl::Event& exit, silas::CallbackData* data)
{
    ...
    pearl::timestamp_t time_to_next_event = exit.next()->getTimestamp() +
                data->get_delta_t() - exit->getTimestamp();
...
//    time_to_next_event = 1.e-6;
    ...
    printf("Shifting events: old delta_t: %e new_delta_t: %e diff: %e\n",
            delta_t, delta_t - (exit->getTimestamp() - enter->getTimestamp()),
    ...
    */
    ...
    event_t      creqt = transfer->isOfType(MPI_RECV_REQUEST) ? MPI_RECV_COMPLETE : MPI_SEND_COMPLETE;

    while (ev_it.is_valid()) {
        if (ev_it->isOfType(MPI_REQUEST_TESTED) && ev_it->getRequestId() == requestId)
            check_call_removal(ev_it, data);

        if (ev_it->isOfType(creqt) && ev_it->getRequestId() == requestId) {
            check_call_removal(ev_it, data);
            break;
        }
    }
        ...
        pearl::CallbackData* cdata)
{
    ...
    MPI_Recv(&delete_msg, 1, MPI_INT, 
             recv->getSource(),
             recv->getTag(), comm->getComm(), MPI_STATUS_IGNORE);
    //printf("Post recv\n");
