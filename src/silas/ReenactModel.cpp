...
void ReenactModel::register_callbacks(const int run, pearl::CallbackManager* cbmanager)
{
    switch(run)
    {
        case 0: // Preprocessing
        {
            ...
            //cbmanager->register_callback(MPI_COLLECTIVE_END, 
        ...
        case 1: // Idle time aggregation
        {
            ...
            //cbmanager->register_callback(GROUP_ALL, 
...
//         }
    ...
    data->save_ts(true, event->getTimestamp());

    // ----- Handling of hypothesis callbacks  --------------------------
    
    // If the region is atomic, we throw the corresponding signal
    if ((event->isOfType(LEAVE)) && (event.prev()->isOfType(GROUP_ENTER)))
    ...
    if (event_type == MPI_COLLECTIVE_END)
    {
        ...
        size_t commsize = event->getComm()->getGroup().numRanks();
        ...
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    ...
    int tag           = prev->getTag();
    MPI_Comm mpicomm  = comm->getComm();

    // set enter event timestamp
    //event.prev().prev()->setTimestamp(get_wtime() - reference_timestamp);
        ...
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    ...
    while (!ev_it->isOfType(GROUP_ENTER)) {
        ...
        if (reqit != reqmap.end()) {
            ...
            if (ev_it->isOfType(GROUP_RECV) || ev_it->isOfType(MPI_SEND_COMPLETE)) {
                compev = ev_it;
                compit = reqit;
            }
...
//            compit->first, ((unsigned*) &compit->second.request)[0]);
    ...
    if (count) {
        ...
        for (++ev_it; !ev_it->isOfType(LEAVE); ++ev_it)
        ...
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    ...
    // set enter event timestamp
    // event.enterptr()->setTimestamp(get_wtime() - reference_timestamp);
...
void ReenactModel::cb_mpi_barrier(const pearl::CallbackManager& cbmanager, 
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    MPI_Comm mpicomm = event->getComm()->getComm();
        ...
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    ...
    int me           = comm->getGroup().getLocalRank(::rank);
    int count        = 0;
        ...
        int user_event, const pearl::Event& event, pearl::CallbackData* cdata)
{
    ...
    data->save_ts(false, event->getTimestamp()); 
}
...
{
    if (!event->isOfType(LEAVE))
        return false;

    // Determine if a wait or test region contains completion events

    for (pearl::Event ev_it = event.prev(); !ev_it->isOfType(GROUP_ENTER); --ev_it)
        if (ev_it->isOfType(GROUP_RECV) || ev_it->isOfType(MPI_SEND_COMPLETE))
            return true;
...
{
    /* FIXME: Use the high resolution timer of the measurement system. */
    return MPI_Wtime();
