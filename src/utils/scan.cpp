...
#ifdef SCAN_EPIK
const string expt_prefix="epik_";
const string expt_tracedir="/ELG";
#else
/*const*/ string expt_prefix="scorep_";
...
int parse_launch (string launcher, bool measure, int argc, char* argv[])
{
    ...
    while ((ch = getopt_long_only(argc, argv, "-:", unified_options, &opti)) != -1) {
        if ((optind>=argc) && (launch_options.target==NULL) && (ch!=TARGET) && measure && (launcher != "ibrun.symm")) {
        ...
        switch(ch) {
            ...
            case CNMODE:
                ...
                if (launcher != "qsub") {
                    if (strchr(argv[optind-1],'=') != NULL) // "--value=val" option format
                        launch_options.other += spc + argv[optind-1];
                    else
            ...
            case NTHRDS:
                launch_options.other += spc + argv[optind-2] + spc + optarg;
                if (launcher != "llrun") break; // generally "-t" specifies time-limit
            ...
            case MICCMD:
                if (launcher != "ibrun.symm") { // something else to be ignored
                    ...
                    if (verbose>1) cerr << "dual_mic=" << dual_mic 
            ...
            case IGNORE: // arg with parameter to explicitly ignore
                if (strchr(argv[optind-1],'=') != NULL) { // "--value=val" option format
                    launch_options.other += spc + argv[optind-1];
                    if (verbose>1) cerr << "Ignoring launcher arg \"" << argv[optind-1] << "\"" << endl;
                } else {
                    launch_options.other += spc + argv[optind-2] + spc + optarg;
...
bool shared_library_linked (const char* executable, const char* library)
{
    string ldd_command;
    ldd_command += "ldd " + string(executable) + " 2>&1"
                        + " | grep -c " + string(library) + " > /dev/null";
    if (verbose>1) cerr << ldd_command << endl;
    int retval = system(ldd_command.c_str());
    if (retval == 2) cerr << SCAN << ldd_command << endl
                          << "Warning: " << strerror(errno) << endl;
    return (retval == 0);
}
...
int main (int argc, char* argv[])
{
...
#else
    char* env_summary=getenv("SCOREP_ENABLE_PROFILING");
    bool summary=(env_summary==NULL) || (env_summary && ((atoi(env_summary)!=0) || (strcmp("true",env_summary)==0)));
    char* env_tracing=getenv("SCOREP_ENABLE_TRACING");
    bool tracing=env_tracing && ((atoi(env_tracing)!=0) || (strcmp("true",env_tracing)==0));
    ...
    if (getenv("SCAN_MPI_REDIRECT")) {
        ...
        if (verbose) cerr << "Set SCAN_MPI_REDIRECT=" << redirect << endl;
    }
#endif
...
//      cerr << "summarization with trace collection & analysis";
...
#else
        bool epk_inst = symbol_in_executable(argv[optind], "SCOREP_InitMeasurement") ||
                        shared_library_linked(argv[optind], "libscorep_");
        if (verbose) cerr << (epk_inst ? "" : "No ")
    ...
    if (launcher == "ibrun.symm" && launch_options.target2 /*measure || archive.empty()*/) {
        ...
        bool epk_inst = symbol_in_executable(target.c_str(), "SCOREP_InitMeasurement") ||
...
#else
        epk_inst_mpi = symbol_in_executable(target.c_str(), "scorep_mpi_status_size") ||
...
#else
        epk_inst_omp = symbol_in_executable(target.c_str(), "POMP2_Parallel_fork") || 
                       shared_library_linked(target.c_str(), "libscorep_mpi_omp.so") ||
...
#ifndef SCAN_EPIK
        epk_inst_pthreads = symbol_in_executable(target.c_str(), "SCOREP_Subsystem_PthreadAdapter") || 
                       shared_library_linked(target.c_str(), "libscorep_adapter_pthread_mgmt.so");
        if (verbose) cerr << (epk_inst_pthreads ? "" : "No ")
    ...
    if (1 /*measure || archive.empty()*/) {
        ...
        if (!launch_options.target && !launch_options.target2) {
    ...
    if (epk_inst_mpi && !launch_options.nprocs) {
    ...
    if (epk_inst_mpi && (launcher=="mpiexec") && !launch_options.nprocs) {
        ...
        if (getenv("PBS_NODEFILE")) {
            ...
            if (verbose>1) cerr << query << endl;
            int retval = system(query.c_str());
            if (retval == 2) cerr << SCAN << query << endl
                                  << "Warning: " << strerror(errno) << endl;
    ...
    if (env_title && archive.empty()) archive = env_title;
#endif
    if (!archive.empty()) { // override title (and EPK_GDIR) with specified archive
        ...
        if ((prefix_start == string::npos) || ((prefix_start > 0) && (path_split != prefix_start-1))) {
    ...
    if (epk_title.empty()) { // default to override
        ...
        if (launch_options.target) {
            ...
            else if (epk_inst_pthreads) epk_title += "xP";
            if (!mic_config.empty()) epk_title += "+";
        } else if (launch_options.target2) { // ibrun.symm -m target2
...
#ifdef SCAN_EPIK // probably not relevant for SCOREP
    ...
    epk_archive += string("/") + expt_prefix + epk_title;
...
#else
    if (verbose) cerr << "SCOREP_EXPERIMENT_DIRECTORY=" << epk_archive.c_str() << endl;
    ...
    if (measure) {
        if (epk_archive_exists(epk_archive.c_str())) {
            ...
            if ((getenv("SCAN_OVERWRITE") && epk_str2bool(getenv("SCAN_OVERWRITE"))) || overwrite) {
    ...
    if (!measure || tracing) { // locate trace analyzer
        ...
        if (getenv("SCALASCA_DIR")) path += string(getenv("SCALASCA_DIR")) + "/bin/backend:";
        ...
        if (analyzer.empty() && (epk_inst_omp || epk_inst_pthreads) && epk_inst_mpi) {
            analcmd = "scout";
    ...
    if (measure) { // update defaults by setting environment variables
        ...
        if (retval) cerr << "setenv(SCOREP_ENABLE_PROFILING): " << strerror(errno) << endl;
        retval = setenv("SCOREP_ENABLE_TRACING", (tracing ? "true" : "false"), 1);
        if (retval) cerr << "setenv(SCOREP_ENABLE_TRACING): " << strerror(errno) << endl;
#endif
...
#else
            if (verbose) cerr << "SCOREP_FILTERING_FILE=" << filter.c_str() << endl;
    ...
    if (epk_archive_exists("/bgl/BlueLight") || epk_archive_exists("/bgsys")) {
        ...
        if (launcher != "srun") { /* configure for mpirun or runjob */
            envflag="-env"; envassign="=";
            if (bgq) envflag="--envs";
    ...
    if (!envflag.empty()) {
        ...
        // string cmd = "scorep-info config-vars --values 2>/dev/null";
        ...
        if (!pipe)
            ...
            while (fgets(confvar, 1024, pipe)) {
                ...
                *val='\0'; val++;
                if (verbose>1) cerr << "Got config-var: " << confvar << envassign << val << endl;
    ...
    if (measure) {
        ...
        if (!escape.empty()) collect += spc + escape;
...
#ifndef SCAN_EPIK // filter archived automatically by EPIK but not SCOREP
            // archive filter
            if (!filter.empty() && access(filter.c_str(), R_OK) == 0) {
                string cp_command = "cp -p " + filter + spc + epk_archive + "/scorep.filt";
                retval = system(cp_command.c_str());
                if (retval) cerr << SCAN << cp_command << endl 
                                 << "Warning: " << strerror(errno) << endl;
                else if (verbose) cerr << "Archived " << filter << endl;
            }
#endif
...
#ifdef SCAN_EPIK // no SCOREP/OTF2 experiment lock
    ...
    if (!measure || (!preview && measure && tracing)) {
        ...
        if (verified && epk_inst_mpi && (launcher!="ibrun.symm")) { // check number of process ranks
...
#ifdef SCAN_EPIK
            const string printer = "elg_print";
#else
            const string printer = "otf2-print";
#endif
            string scan = string(argv[0]);
            string print_path = which(printer, string(BINDIR) + ":"
            ...
            if (print_path.empty()) {
                ...
                string cmd = print_path + opt + edeffile + " 2>/dev/null | grep -c LOCATION_GROUP.*MPI";
#endif
                FILE *pipe = popen(cmd.c_str(),"r");
                if (!pipe)
                    cerr << SCAN << "Error: " << cmd << endl << strerror(errno) << endl;
                ...
                if (!pipe)
                    ...
                    fscanf(pipe, "%d", &openmp);
                    if (verbose) cerr << "Got paradigm OpenMP=" << openmp << " from " << edeffile << endl;
                    pclose(pipe);
                }
#endif
            }
    ...
    if ((!measure || (measure && tracing)) && (analcmd != "none")) {
        ...
        if (!escape.empty()) analyze += spc + escape;
...
#ifndef SCAN_EPIK // explicitly specify OTF2 handle
        ...
        if (launcher == "runjob") { // remove "--mapping <file|spec>"
          string flag = "--mapping ";
          size_t start = analyze.find(flag);
          if (start != string::npos) {
            size_t end = analyze.find_first_of(' ',start+flag.length()+1);
            if (end != string::npos) {
              if (verbose) cerr << "Eliminating incompatible runjob option: "
