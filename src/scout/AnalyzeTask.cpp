...
class AnalysisReplayTask : public pearl::ReplayTask
{
public:
  typedef enum {
    forward, backward
...
{
  for (PatternList::iterator it = mPatterns.begin(); it != mPatterns.end(); ++it)
    delete *it;
}
 ...
 **/
void AnalyzeTask::addPattern(Pattern* pattern)
{
 ...
 **/
void AnalyzeTask::genCubeMetrics(ReportData& data) const
{
  // Create performance metrics
  for (PatternList::const_iterator it = mPatterns.begin(); it != mPatterns.end(); ++it) {
 ...
 **/
void AnalyzeTask::genCubeSeverities(ReportData& data,
                                    int         rank) const
{
  for (PatternList::const_iterator it = mPatterns.begin(); it != mPatterns.end(); ++it)
