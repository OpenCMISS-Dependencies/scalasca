...
public:
    ...
    fixed_set() 
        : m_min(0), m_max(0), m_rep(one) { 
        m_data.entry = m_max + 1;
    }

    fixed_set(const Key& min, const Key& max) 
        : m_min(min), m_max(max), m_rep(one) { 
        m_data.entry = m_max + 1;
    }
    ...
    public:
        ...
        iterator(Key pos, const fixed_set<Key>* set)
            : m_pos(pos), m_set(set) { }
        ...
        iterator operator ++ () {
            ...
            case one_inverse:
                if (m_pos == m_set->m_data.entry && m_pos <= m_set->m_max)
                    ++m_pos;
            ...
            case full:
            {
                ...
                while (!bv.contains(m_pos) && m_pos <= m_set->m_max)
                    ++m_pos;
            }
                break;
            }
        ...
        Key operator *() const {
            if (m_set->contains(m_pos)) // bad call -> crash!
                return m_pos;
        ...
        bool operator == (const iterator& i) const {
            ...
            if (m_pos > m_set->m_max && i.m_pos > m_set->m_max)
                return true;
    ...
    fixed_set<Key>& operator = (const fixed_set<Key>& s) {
        copy(s);
        return *this;
    }
    ...
    bool empty() const {
        ...
        case one_inverse:
            return m_min == m_max && m_data.entry == m_min;
        case full:
    ...
    fixed_set<Key> intersection_with(const fixed_set<Key>& fset) {
...
private:
    ...
    void copy(const fixed_set<Key>& s) {
        if (this == &s)
            return;
    ...
    public:
        ...
        bitvector& copy(const fixed_set<Key>& s) {
        ...
        void detach() {
            if (p->m_data.bitvec && --(p->m_data.bitvec->rcount) == 0) {
    ...
    friend bool operator >= <Key> (const fixed_set<Key>& a, const fixed_set<Key>& b);
};

template<class Key>
bool operator >= (const fixed_set<Key>& a, const fixed_set<Key>& b) {
    if (b.empty())
        return true;
    if (a.empty())
        return false;

    if (a.m_min > b.m_max ||  a.m_max < b.m_min)
        return false;
    ...
    case fixed_set<Key>::one:
        ...
        case fixed_set<Key>::one_inverse:
            return b.m_min == b.m_max && b.m_data.entry != a.m_data.entry;
        case fixed_set<Key>::full:
            for (typename fixed_set<Key>::iterator it = b.begin(); it != b.end(); ++it)
    ...
    case fixed_set<Key>::one_inverse:
        if (a.m_data.entry > a.m_max) {
            ...
            switch (b.m_rep) {
            case fixed_set<Key>::one:
                return b.m_data.entry >= a.m_min && b.m_data.entry <= a.m_max;
            case fixed_set<Key>::one_inverse:
                if (b.m_max > a.m_max || b.m_min < a.m_min)
                    return false;
            ...
            case fixed_set<Key>::one_inverse:
                if (b.m_max > a.m_max || b.m_min < a.m_min)
                    return false;
                if (b.m_data.entry <= b.m_max && b.m_data.entry != a.m_data.entry)
                    return false;
...
template<class Key>
fixed_set<Key> operator - (const fixed_set<Key>& a, const fixed_set<Key>& b) {
