...
struct TmapCacheHandler::TmapCHImpl
{
    ...
    timemap_t getRuntimes(const Event& from, const Event& to) const {
    ...
    void save_synchpoint(const Event& event) {
        // --- update tmap & cmap cache

        for (timemap_t::const_iterator it = mTmap.begin(); it != mTmap.end(); ++it) {
    ...
    register_pre_pattern_callbacks(const CallbackManagerMap& cbm)
    {
        ...
        {
            pearl::event_t evt;
            void (TmapCHImpl::*cbp)(const pearl::CallbackManager &, int, const pearl::Event &, pearl::CallbackData*);
        };
        ...
        const struct cb_evt_table_t fws_evt_tbl[] = {
            { ENTER,              &TmapCHImpl::cb_fws_pre_enter },
#ifdef _MPI
...
#ifdef _MPI
            { INIT_END,           &TmapCHImpl::cb_fws_pre_mpi_synchpoint },
            { FINALIZE_END,       &TmapCHImpl::cb_fws_pre_mpi_synchpoint },
#endif
#ifdef _OPENMP
    ...
    register_post_pattern_callbacks(const CallbackManagerMap& cbm)
    {
        ...
        const struct cb_evt_table_t fws_evt_tbl[] = {
            { LEAVE,              &TmapCHImpl::cb_fws_post_leave     },
...
timemap_t TmapCacheHandler::getRuntimes(const Event& from, const Event& to) const
{
