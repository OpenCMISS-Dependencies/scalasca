...
void CalculateOmpBarrierDelay(const TimeRank& my,
                              ...
                              CbData*         data)
{
    ...
    data->mInputScales.insert(std::make_pair(KEY_WAITTIME, static_cast<double>(latest.time - my.time)));

    for (map<uint32_t, double>::iterator inpIt = data->mInputScales.begin(); inpIt != data->mInputScales.end(); ++inpIt) {
#pragma omp barrier
        { }
        if (my.rank == latest.rank) {
...
#pragma omp barrier
        ...
        if (diffMap.mSum + propWait > 0)
            scale = std::min(inpIt->second / (diffMap.mSum + propWait), 1.0);
