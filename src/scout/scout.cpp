...
/// Flag indicating whether asynchronous message handling (a.k.a. active message)
...
void Initialize(int* argc, char*** argv)
{
  // FIXME: Ugly hack to get EPIK configuration variable handling initialized
  elg_cntl_msg("Initialization...");
  ...
  // Initialize PEARL
  #if defined(_MPI)   // MPI or hybrid MPI+OpenMP
    PEARL_mpi_init();
...
void ProcessArguments(int argc, char** argv)
{
  ...
  while (index < argc) {
    ...
    // Deprecated options
    if ("-i" == arg) {
      LogMsg(0, "Ignoring deprecated command-line option '-i':\n"
                "    Instance tracking and statistics now enabled by default.\n\n");
    } else if ("-s" == arg) {
      LogMsg(0, "Command-line option '-s' is deprecated, use '--time-correct' instead.\n\n");
      #if defined(_MPI)
        synchronize = true;
      #endif   // _MPI
    }
    ...
    } else if ("--statistics" == arg) {
      enableStatistics = true;
    } else if ("--no-statistics" == arg) {
      enableStatistics = false;
    } else if ("--critical-path" == arg) {
      enableCriticalPath = true;
    } else if ("--no-critical-path" == arg) {
      enableCriticalPath = false;
    } else if ("--rootcause" == arg) {
      enableDelayAnalysis = true;
    } else if ("--no-rootcause" == arg) {
      enableDelayAnalysis = false;
    } else if ("--single-pass" == arg) {
      enableSinglePass = true;
      LogMsg(0, "Using single-pass replay.\n"
...
#if defined(_MPI)
    else if ("--time-correct" == arg) {
      synchronize = true;
    } else if ("--no-time-correct" == arg) {
      synchronize = false;
    }
#endif   // _MPI
    ...
    // Unknown option
      ...
      #if !defined(_MPI)
        if ("--time-correct" == arg || "--no-time-correct" == arg) {
...
void Usage(const char* command)
{
  ...
  LogMsg(0, "Usage: %s%s [OPTION]... <ANCHORFILE | EPIK DIRECTORY>\n"
            "Options:\n"
                 ...
                 TaskDataPrivate& privateData)
{
  ...
  writer = new ReportWriterTask(sharedData, analyzer, archiveDirectory, rank);
...
void Analyze()
{
  ...
  // Eventually go hybrid...
    ...
    const LocationGroup& group = defs.getLocationGroup(rank);
