...
/// @param [in] epoch      List of events in the epoch
                               ...
                               Buffer&             buffer)
{
    ...
    buffer.put_uint32(comm.getGroup().getLocalRank(globalRank));
                               ...
                               AmRequest*          request)
{
    ...
    pack(key, comm, dependency, epoch, *buffer);
}

/// Helper struct for PGI compiler
struct timestamp_comp {
    bool operator()(const pearl::Event_rep& e, const pearl::timestamp_t t) {
        return e < t;
    }
};
...
AmWaitForProgressRequest::execute(Buffer& buffer)
{
    ...
    assert((*remote_it)->isOfType(ENTER));
    ...
    assert((*remote_it)->isOfType(LEAVE));
    ...
    if ((dependency == SINGLE_DEPENDENCY)
        ...
        cbdata->mSyncRank = comm->getGroup().getGlobalRank(remote);
