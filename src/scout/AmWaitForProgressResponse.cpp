...
AmWaitForProgressResponse::execute(Buffer& buffer)
{
    ...
    if (element.numPending() == 0) {
        ...
        while (remote_segment != element.getRemoteSegments().end()) {
            //cerr << "current_local_start = " << current_local_start << endl;
            //cerr << "Remote Segment (" << remote_segment->get_start() << ":" << remote_segment->get_end() << ")" << endl;
            if (remote_segment->getStart() > current_local_start) {
                cbdata->mIdle += remote_segment->getStart() - current_local_start;

                //cerr << "Idle Time " << cbdata->mIdle << endl;
            }
        ...
        if (cbdata->mIdle > 0) {
            ...
            ContentionInfo info = { event_id, 0, comm->getGroup().getGlobalRank(remote_rank) };
