...
event_t MpiSendRequest_rep::getType() const
{
  return MPI_SEND_REQUEST;
}


bool MpiSendRequest_rep::isOfType(event_t type) const
{
  return ((MPI_SEND_REQUEST == type)
          || (GROUP_SEND == type)
          || (GROUP_NONBLOCK == type)
          || (GROUP_ALL == type));
}
...
uint64_t MpiSendRequest_rep::getRequestId() const
{
  return mRequestId;
}
...
void MpiSendRequest_rep::setRequestId(uint64_t requestId)
{
  mRequestId = requestId;
}
