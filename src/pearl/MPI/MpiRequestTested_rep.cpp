...
event_t MpiRequestTested_rep::getType() const
{
  return MPI_REQUEST_TESTED;
}


bool MpiRequestTested_rep::isOfType(event_t type) const
{
  return ((MPI_REQUEST_TESTED == type)
          || (GROUP_NONBLOCK == type)
          || (GROUP_ALL == type));
}
...
uint64_t MpiRequestTested_rep::getRequestId() const
{
  return mRequestId;
}
...
void MpiRequestTested_rep::setRequestId(uint64_t requestId)
{
  mRequestId = requestId;
}
...
ostream& MpiRequestTested_rep::output(ostream& stream) const
{
