...
namespace
{
    ...
    uint32_t classify(const string& name, const string& file, Region::Role role, Paradigm paradigm);
    uint32_t classify_omp(const string& name);
    uint32_t classify_mpi(const string& name);
    uint32_t classify_pthread(const string& name);
    // *INDENT-ON*
}   // unnamed namespace
           ...
           const Region& item)
{
    ...
    stream << iendl(indent)
           ...
           << "  role        = " << toString(item.getRole()) << iendl(indent)
           << "  paradigm    = " << item.getParadigm() << iendl(indent)
         ...
         Paradigm      paradigm)
{
    ...
    if ((paradigm == Paradigm::MEASUREMENT_SYSTEM) || (file == "EPIK")) {
...
classify_pthread(const string& name)
{
    ...
    if (pthreadName == "mutex_lock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_MUTEX | Region::cMODE_PTHREAD_LOCK;
    }
    if (pthreadName == "mutex_trylock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_MUTEX | Region::cMODE_PTHREAD_TRYLOCK;
    }
    if (pthreadName == "mutex_unlock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_MUTEX | Region::cMODE_PTHREAD_UNLOCK;
    }
    if (pthreadName == "spin_lock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_SPIN | Region::cMODE_PTHREAD_LOCK;
    }
    if (pthreadName == "spin_trylock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_SPIN | Region::cMODE_PTHREAD_TRYLOCK;
    }
    if (pthreadName == "spin_unlock") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_SPIN | Region::cMODE_PTHREAD_UNLOCK;
    }
    if (pthreadName == "cond_timedwait") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_CONDITION | Region::cMODE_PTHREAD_TIMEDWAIT;
    }
    if (pthreadName == "cond_wait") {
        return Region::cCLASS_PTHREAD | Region::cCAT_PTHREAD_SYNC
               | Region::cTYPE_PTHREAD_CONDITION | Region::cMODE_PTHREAD_WAIT;
    }
